...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null) {
        ...
        set_time_limit(intval($size) / 1048576); //Allows script to run for n seconds longer, where n is the size of the file in MB
        $file->type = $type;
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                $mysqli = new mysqli("localhost", "filebox", "3Y3SUKzQVKHav8fQ", "filebox");
                $fileName = $mysqli->real_escape_string($name);
                $fileSize = $mysqli->real_escape_string($size);
                $fileType = $mysqli->real_escape_string($type);
                $fp = fopen($uploaded_file, 'r');
                $res = $mysqli->query("INSERT INTO files (File_Name, File_Type, File_Size, File_Content) VALUES 
                    ('" . $fileName . "', '" . $fileType . "', '" . $fileSize . "', '" . $mysqli->real_escape_string(fread($fp, filesize($uploaded_file))) . "')");
                fclose($fp);
